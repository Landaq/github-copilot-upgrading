...
class TestDbdict(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(foo.select_value,
                         'SELECT value FROM _guachi_data WHERE key=?')
        self.assertEqual(
            foo.select_key, 'SELECT key FROM _guachi_data WHERE key=?')
        self.assertEqual(foo.update_value,
                         'UPDATE _guachi_data SET value=? WHERE key=?')
        self.assertEqual(foo.insert_key_value,
                         'INSERT INTO _guachi_data (key,value) VALUES (?,?)')
        self.assertEqual(
            foo.delete_key, 'DELETE FROM _guachi_data WHERE key=?')
    ...
    def test_setitem_typeerror(self):
        foo = database.dbdict('/tmp/test_guachi')
        self.assertRaises(sqlite3.InterfaceError,
                          foo.__setitem__, 'bar', {'a': 'b'})
...
# Direct copy from legacy/guachi/tests/test_database.py
